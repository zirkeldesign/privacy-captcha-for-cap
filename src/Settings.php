...
class Settings
{
    ...
    public function enqueueAdminAssets(string $hookSuffix): void
    {
        ...
        wp_add_inline_script(
            ...
            'window.capCaptchaAdmin = '.wp_json_encode([
                ...
                'i18n' => [
                    'testing' => __('Testing…', 'cap-captcha'),
                    'ok' => __('Connection OK', 'cap-captcha'),
                    'error' => __('Connection failed', 'cap-captcha'),
                ],
            ]).';',
            'before'
        );
    ...
    public function ajaxTestConnection(): void
    {
        ...
        if (! current_user_can(self::CAPABILITY)) {
            wp_send_json_error(['message' => __('You do not have permission to run this check.', 'cap-captcha')], 403);
        }

        if (! $this->isConfigured()) {
            wp_send_json_error(['message' => __('Endpoint / Site key / Secret key not yet configured.', 'cap-captcha')]);
        }

        if ($this->getAdminApiKey() === '') {
            wp_send_json_error(['message' => __('Admin API key is empty — connection test needs it.', 'cap-captcha')]);
        }

        $stats = (new StatsClient($this))->fetch(true);

        if ($stats === null) {
            wp_send_json_error(['message' => __('Cap server did not return a valid response. Verify endpoint and admin API key.', 'cap-captcha')]);
        }
        ...
        wp_send_json_success([
            'message' => sprintf(
                ...
                __('Connected — site "%1$s" (%2$s)', 'cap-captcha'),
                $name !== '' ? $name : '—',
                $siteKey
            ),
        ]);
    ...
    public function sanitize($input): array
    {
        ...
        $secretKey = $this->isSecretKeyConstantSet()
            ...
            : sanitize_text_field((string) ($input['secret_key'] ?? ''));
        $adminApiKey = $this->isAdminApiKeyConstantSet()
            ? (string) ($stored['admin_api_key'] ?? '')
            : sanitize_text_field((string) ($input['admin_api_key'] ?? ''));
    ...
    {
        if (! current_user_can(self::CAPABILITY)) {
            wp_die(esc_html__('You do not have permission to access this page.', 'cap-captcha'));
        }

        $v = $this->getAll();
        ?>
        <div class="wrap">
            <h1><?php echo esc_html__('Cap CAPTCHA', 'cap-captcha'); ?></h1>
            <p>
                <?php
                printf(
                    /* translators: %s is a link to trycap.dev */
                    esc_html__('Connect WordPress to your self-hosted Cap server and choose which forms to protect. See %s for server setup.', 'cap-captcha'),
        ...
        ?>
            ...
            <form method="post" action="options.php">
                ...
                <h2><?php echo esc_html__('Cap server', 'cap-captcha'); ?></h2>
                <table class="form-table" role="presentation">
                    <?php $endpointLocked = $this->isEndpointConstantSet(); ?>
                    <tr>
                        <th scope="row"><label for="cap_endpoint_base"><?php echo esc_html__('Endpoint base URL', 'cap-captcha'); ?></label></th>
                        <td>
                            <input
                                type="url"
                            ...
                            <p class="description">
                                <?php if ($endpointLocked) { ?>
                                    <?php echo esc_html__('Defined via CAP_CAPTCHA_ENDPOINT in wp-config.php — value here is read-only.', 'cap-captcha'); ?>
                                <?php } else { ?>
                                    <?php echo esc_html__('Public URL of your Cap server, e.g. https://cap.example.com/', 'cap-captcha'); ?>
                                <?php } ?>
                            </p>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <th scope="row"><label for="cap_site_key"><?php echo esc_html__('Site key', 'cap-captcha'); ?></label></th>
                        <td>
                            <input
                                type="text"
                                id="cap_site_key"
                            ...
                            <p class="description">
                                <?php if ($siteKeyLocked) { ?>
                                    <?php echo esc_html__('Defined via CAP_CAPTCHA_SITE_KEY in wp-config.php — value here is read-only.', 'cap-captcha'); ?>
                                <?php } else { ?>
                                    <?php echo esc_html__('Public site key issued by your Cap server.', 'cap-captcha'); ?>
                                <?php } ?>
                            </p>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <th scope="row"><label for="cap_secret_key"><?php echo esc_html__('Secret key', 'cap-captcha'); ?></label></th>
                        <td>
                            <input
                                type="password"
                                id="cap_secret_key"
                            ...
                            <p class="description">
                                <?php if ($secretLocked) { ?>
                                    <?php echo esc_html__('Defined via CAP_CAPTCHA_SECRET_KEY in wp-config.php — value here is read-only.', 'cap-captcha'); ?>
                                <?php } else { ?>
                                    <?php echo esc_html__('Secret key used to call /siteverify. Define CAP_CAPTCHA_SECRET_KEY in wp-config.php to override.', 'cap-captcha'); ?>
                                <?php } ?>
                            </p>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <th scope="row"><label for="cap_admin_api_key"><?php echo esc_html__('Admin API key', 'cap-captcha'); ?></label></th>
                        <td>
                            <input
                                type="password"
                            ...
                            <p class="description">
                                <?php if ($apiLocked) { ?>
                                    <?php echo esc_html__('Defined via CAP_CAPTCHA_ADMIN_API_KEY in wp-config.php — value here is read-only.', 'cap-captcha'); ?>
                                <?php } else { ?>
                                    <?php echo esc_html__('Optional. Issue a dedicated key under Settings → API Keys in your Cap server and paste it here. Used for the Status panel below. Define CAP_CAPTCHA_ADMIN_API_KEY in wp-config.php to override.', 'cap-captcha'); ?>
                                <?php } ?>
                            </p>
                            <p class="cap-captcha-test-connection">
                                <button type="button" class="button" data-cap-captcha-test>
                                    <?php echo esc_html__('Test connection', 'cap-captcha'); ?>
                                </button>
                ...
                <h2><?php echo esc_html__('Display', 'cap-captcha'); ?></h2>
                <table class="form-table" role="presentation">
                    <tr>
                        <th scope="row"><label for="cap_display_mode"><?php echo esc_html__('Default display mode', 'cap-captcha'); ?></label></th>
                        <td>
                            <select id="cap_display_mode" name="<?php echo esc_attr(self::OPTION_KEY); ?>[display_mode]">
                                <option value="<?php echo esc_attr(self::MODE_INLINE); ?>" <?php selected($v['display_mode'], self::MODE_INLINE); ?>><?php echo esc_html__('Inline — widget visible in the form', 'cap-captcha'); ?></option>
                                <option value="<?php echo esc_attr(self::MODE_FLOATING); ?>" <?php selected($v['display_mode'], self::MODE_FLOATING); ?>><?php echo esc_html__('Floating — opens on click of a trigger button', 'cap-captcha'); ?></option>
                                <option value="<?php echo esc_attr(self::MODE_PROGRAMMATIC); ?>" <?php selected($v['display_mode'], self::MODE_PROGRAMMATIC); ?>><?php echo esc_html__('Programmatic — solves silently in the background (no user interaction)', 'cap-captcha'); ?></option>
                            </select>
                            <p class="description"><?php echo esc_html__('Per-form integrations can override this default.', 'cap-captcha'); ?></p>
                        </td>
                    </tr>
                </table>

                <h2><?php echo esc_html__('Privacy', 'cap-captcha'); ?></h2>
                <table class="form-table" role="presentation">
                    <tr>
                        <th scope="row"><?php echo esc_html__('WASM source', 'cap-captcha'); ?></th>
                        <td>
                            <fieldset>
                                <label>
                                    ...
                                    <?php echo esc_html__('Bundled — load from this plugin (recommended, DSGVO-clean)', 'cap-captcha'); ?>
                                </label><br>
                                <label>
                                    ...
                                    <?php echo esc_html__('Cap server — load from your endpoint at /assets/cap_wasm_bg.wasm', 'cap-captcha'); ?>
                                </label><br>
                                <label>
                                    ...
                                    <?php echo esc_html__('jsdelivr CDN — fastest globally, but leaks visitor IPs to a third party', 'cap-captcha'); ?>
                                </label>
                            </fieldset>
                        </td>
                    </tr>
                ...
                <h2><?php echo esc_html__('Integrations', 'cap-captcha'); ?></h2>
                            ...
                            <label class="cap-captcha-integration <?php echo $meta['available'] ? '' : 'cap-captcha-integration--disabled'; ?>">
                                ...
                                <span class="cap-captcha-integration__body">
                                    ...
                                    <?php if (! $meta['available']) { ?>
                                        <span class="cap-captcha-integration__status"><?php echo esc_html__('Plugin not active', 'cap-captcha'); ?></span>
                                    <?php } ?>
                                </span>
                            </label>
                        <?php } ?>
                ...
                <h2><?php echo esc_html__('Behavior', 'cap-captcha'); ?></h2>
                <table class="form-table" role="presentation">
                    <tr>
                        <th scope="row"><?php echo esc_html__('Fail-open mode', 'cap-captcha'); ?></th>
                        <td>
                            <label>
                                <input type="checkbox" name="<?php echo esc_attr(self::OPTION_KEY); ?>[fail_open]" value="1" <?php checked($v['fail_open']); ?>>
                                <?php echo esc_html__('Let submissions through when the Cap server is unreachable', 'cap-captcha'); ?>
                            </label>
                            <p class="description"><?php echo esc_html__('Off by default. Enable only if temporary Cap outages must not block legitimate users (logins, checkouts).', 'cap-captcha'); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row"><?php echo esc_html__('Admin notices', 'cap-captcha'); ?></th>
                        <td>
                            <label>
                                <input type="checkbox" name="<?php echo esc_attr(self::OPTION_KEY); ?>[show_admin_notices]" value="1" <?php checked($v['show_admin_notices']); ?>>
                                <?php echo esc_html__('Warn site admins inline when an enabled integration is not configured', 'cap-captcha'); ?>
                            </label>
                            <p class="description"><?php echo esc_html__('Only users with manage_options ever see these notices — anonymous visitors get nothing rendered.', 'cap-captcha'); ?></p>
                        </td>
                    </tr>
                </table>

                <?php submit_button(__('Save settings', 'cap-captcha'), 'primary large'); ?>
            </form>
        </div>
        <?php
    }
    ...
    private function renderStatusPanel(): void
    {
        ...
        echo '<h2>'.esc_html__('Status', 'cap-captcha').'</h2>';

        if ($stats === null) {
            echo '<div class="notice notice-warning inline"><p>'
                .esc_html__('Could not fetch status from the Cap server. Check the Endpoint, Site key, and Admin API key.', 'cap-captcha')
                .'</p></div>';
        ...
        <div class="cap-captcha-status">
            <div class="cap-captcha-status__grid">
                <div class="cap-captcha-status__card">
                    <span class="cap-captcha-status__label"><?php echo esc_html__('Site', 'cap-captcha'); ?></span>
                ...
                <?php
                $this->renderStatCard(
                    __('Challenges', 'cap-captcha'),
                    number_format_i18n($challenges),
                    $challenges,
                    (int) ($prev['challenges'] ?? 0),
                );
        $this->renderStatCard(
            __('Verified', 'cap-captcha'),
            number_format_i18n($verified),
            $verified,
            (int) ($prev['verified'] ?? 0),
        );
        $this->renderStatCard(
            __('Failed', 'cap-captcha'),
            number_format_i18n($failed),
            $failed,
            (int) ($prev['failed'] ?? 0),
        );
        $this->renderStatCard(
            __('Avg. duration', 'cap-captcha'),
            /* translators: %s is the latency in seconds with one decimal, e.g. "3.4 s" */
            sprintf(esc_html__('%s s', 'cap-captcha'), number_format_i18n($avgLatencyMs / 1000, 1)),
            $avgLatencyMs,
            (int) ($prev['avgLatency'] ?? 0),
        );
        ?>
                <div class="cap-captcha-status__card">
                    <span class="cap-captcha-status__label"><?php echo esc_html__('Difficulty', 'cap-captcha'); ?></span>
                    ...
                    <span class="cap-captcha-status__sub">
                        <?php echo esc_html(sprintf(
                            ...
                            __('%1$d challenges · %2$d-byte salt', 'cap-captcha'),
                            $challengeCount,
                            $saltSize
                        )); ?>
                    </span>
            ...
            <p class="cap-captcha-status__refresh">
                <a class="button" href="<?php echo esc_url($refreshUrl); ?>"><?php echo esc_html__('Refresh now', 'cap-captcha'); ?></a>
                <span class="description"><?php echo esc_html__('Status is cached for 5 minutes.', 'cap-captcha'); ?></span>
            </p>
        </div>
    ...
    private function renderSparkline(array $buckets): void
    {
        ...
        echo '<div class="cap-captcha-status__sparkline" role="img" aria-label="'
            .esc_attr__('Hourly solves chart', 'cap-captcha')
            .'">';
        ...
        foreach ($buckets as $bucket) {
            ...
            $title = sprintf(
                ...
                esc_attr__('%1$s — %2$d verified', 'cap-captcha'),
    ...
    private function integrationGroups(): array
    {
        ...
        return [
            [
                'label' => __('WordPress', 'cap-captcha'),
                'description' => __('Protect WordPress core forms.', 'cap-captcha'),
                'items' => array_intersect_key($all, array_flip(['comments', 'login', 'registration'])),
            ],
            [
                'label' => __('Plugins', 'cap-captcha'),
                'description' => __('Protect form-builder and e-commerce plugin submissions.', 'cap-captcha'),
    ...
    private function integrationDescriptions(): array
    {
        return [
            'gravity_forms' => [
                'label' => __('Gravity Forms', 'cap-captcha'),
                'description' => __('Adds a "Cap CAPTCHA" field to the form editor under Advanced Fields.', 'cap-captcha'),
                'available' => class_exists('GFForms') || class_exists('GFAPI'),
            ],
            'comments' => [
                'label' => __('Comments', 'cap-captcha'),
                'description' => __('Protects the default WordPress comment form.', 'cap-captcha'),
                'available' => true,
            ],
            'login' => [
                'label' => __('Login', 'cap-captcha'),
                'description' => __('Protects wp-login.php and any front-end login form using login_form.', 'cap-captcha'),
                'available' => true,
            ],
            'registration' => [
                'label' => __('Registration', 'cap-captcha'),
                'description' => __('Protects the WordPress user registration form.', 'cap-captcha'),
                'available' => true,
            ],
            'woocommerce' => [
                'label' => __('WooCommerce checkout', 'cap-captcha'),
                'description' => __('Protects the WooCommerce checkout form.', 'cap-captcha'),
